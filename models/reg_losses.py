...
shape = (256, 256)
pth_dir = './checkpoints/VGG/pretrain_VGG_923/VGG.pth'
...
class LossFunction_Dense(nn.Module):
    def __init__(self):
        ...
        self.feat_loss = VGGLoss()#TODO VGGLoss()
...
class VGGLoss(nn.Module):
    def __init__(self):
        ...
        if torch.cuda.is_available():
            self.vgg.cuda()
        self.vgg.load_state_dict(torch.load(pth_dir))
        print('-----------------reg_239 load vgg19 model from {}-------------------'.format(pth_dir))
        self.vgg.eval()
